  ...
  const runCode = async () => {
    ...
    for (let i = 0; i < allInputs.length; i++) {
      ...
      try {
        ...
        results.push(output);
       const expected = i < problem.testCases.length
  ? problem.testCases[i].output?.trim().replace(/\s+/g, " ")
  : customCases[i - problem.testCases.length]?.output?.trim().replace(/\s+/g, " ");
        verdictList.push(
  ...
  {finalVerdict === "Accepted" ? (
    <div className="bg-[#222] p-6 rounded shadow-md border border-green-700/30">
      ...
      <div className="text-sm text-gray-400 mb-3">
        Total Passed:{" "}
        <span className="text-green-300">{problem.testCases?.length}</span>
      </div>
      ...
      {showAddInput ? (
            ...
            <button
              onClick={() => {
                if (newInput.trim() && newOutput.trim()) {
                  setCustomCases([
                    ...customCases,
                    { input: newInput.trim(), output: newOutput.trim() },
                  ]);
                  setNewInput("");
                  setNewOutput("");
        ...
        <div className="bg-[#2a2a2a] rounded p-4 mt-4 border border-gray-700 shadow-sm text-sm text-green-300">
          <p style={{ whiteSpace: "pre-wrap" }}>
            <strong>Input:</strong>
            <br />
            {allInputs[selectedCase]?.input || allInputs[selectedCase]}
          </p>
